...
public class Main {
  ...
  BankAccount bankAccount1 = new BankAccount(100010001); // Объект счет в банке
  bankAccount1.print();
  ...
  bankAccount1.setCashFlows(cash);

  if (typeOperation == 1) {
    bankAccount1.pushMoneyAmount();   // Вызов метода перечислить на счет
  }
  if (typeOperation == 2) {
    bankAccount1.pullMoneyAmount();   // Вызов метода снять со счёта
  }
  bankAccount1.print();
  }
}
